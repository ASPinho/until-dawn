...
public class StreetScreen extends AbsctractScreen {
    ...
    private Street parallaxBackground;

    private Skin skin;

    public StreetScreen(Game game, Skin skin) {
        this.game = game;
        this.skin = skin;
        ...
        for (int i = 1; i <= 3; i++) {
            textures.add(new Texture(Gdx.files.internal("parallax/city/city" + i + ".png")));
            textures.get(textures.size - 1).setWrap(Texture.TextureWrap.MirroredRepeat, Texture.TextureWrap.MirroredRepeat);
        }

        parallaxBackground = new Street(textures);
    ...
    public void enterHome(Skin skin) {
        if(player.getX() <= 20 && player.getY() == 50 && Gdx.input.isKeyPressed(Input.Keys.W)) {
           game.setScreen(new HomeScreen(game,this, skin));
        }
    }
    ...
    public void overWorld() {
        ...
        if(player.getX() >= PRE_GAME_BACKGROUND_WIDTH - PICTURE_SIZE) {
            game.setScreen(new OverWorld(game,this, skin));
            returnToCity = true;
        }
    }
}